...
clear
clc
Incub_dur = [3*ones(4,1);14*ones(10,1);2*ones(2,1)]; %6.4;
Infec_dur = [10*ones(4,1);3*ones(10,1);9*ones(2,1)]; % 7;
lambda  = 0; % 0.018/365; % Rate of transmission of disease
t_tot   = 365; % How long will the simulation run
N_tot   = 1.37e9; % Total poulation
rho = [0.8*ones(4,1);0.6*ones(10,1);1;1];
alpha = 0.25; % Prob that infected asymptomatic infects
beta  = 0.8d0; % Prob. that infectious transmits disease to susceptible
kappa = ones(16,1)./Incub_dur;
gamma = ones(16,1)./Infec_dur;
...
w_s = 0.d0*ones(16,1); %[1.5*ones(4,1);1.1*ones(12,1)]; % Effect of lockdown at school
w_w = [0.0*ones(4,1);0.2*ones(10,1);0.2*ones(2,1)]; % Effect of lockdown at work
w_h = [1.8*ones(4,1);1.5*ones(10,1);ones(2,1)]; % Effect of lockdown at home
w_o = [0.1*ones(14,1);0.0*ones(2,1)]; % Effect of lockdown at other places
Nis  = load([pwd '\input\India_age_data.txt']); % Population fractions in age groups
I0   = 200*Nis/N_tot; % Number of infected initially, populated as population age fractions
S0  = 1-1.25*I0; % Initial number of susceptible in age group, E0 = I0
A = [rho,kappa,gamma,mu_n,mu_d,w_s,w_w,w_h,w_o,Nis,I0,S0];
t_l1    = 20; % Lockdown onset
t_l2    = 41; % Lockdown stop
